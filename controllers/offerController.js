		...
		db.Offer
			...
			.then(() => {
				const mailer = new Mailer(
					...
					sender.email,
					'Offer',
					`<p>You have an offer on from ${sender.email} on your property.</p> 
...
const acceptOffer = async (req, res) => {
	...
	if (hasOffer) {
		...
		const mailer = new Mailer(
			...
			sender.email,
			'Offer Accepted',
			`<p>Your offer on the property, ${home.address} has been accepted!</p> 
...
const buyerAcceptOffer = async (req, res) => {
	...
	offer.save().then((doc) => res.json(doc)).catch((err) => res.json(err));
};
...
const getOffersByUserAndHome = async (req, res) => {
	db.Offer
		.aggregate([
			{
				$match: {
...
async function sendSellerSignedPurchaseAgreementMail(offer) {
	...
	const mailer = new Mailer(
		...
		`<p>The seller ${sender.firstName} has signed your purchase agreement!</p> 
        <p>Visit your <a href='localhost:3000/buyerdashboard/${home._id}'>dashboard</a> for the next steps.</p>`
	);
	mailer.sendMail();
}
