...
const viewMessage = async (req, res) => {
	db.Messages
		.update({ 'messages._id': new ObjectId(req.body.messageId) }, { $set: { 'messages.$.viewed': true } })
...
const getMessages = (req, res) => {
	db.Messages.find({ participants: req.params.email }).then((doc) => res.json(doc)).catch((err) => res.json(err));
};
